...
t = t';
%% Pressure data
methane_P1 = methane_arr(1:length(t),1);
h50_P1 = h50_arr(1:length(t),1);
h10_P1 = h10_arr(1:length(t),1);
%% Mean removed
methane_P1 = methane_P1-mean(methane_P1)*ones(length(methane_P1),1);
h50_P1 = h50_P1-mean(h50_P1)*ones(length(h50_P1),1);
h10_P1 = h10_P1-mean(h10_P1)*ones(length(h10_P1),1);
%% Denoising data 
h10_P1f = sgolayfilt(h10_P1,15,201);
h50_P1f = sgolayfilt(h50_P1,15,201);
methane_P1f = sgolayfilt(methane_P1,15,201);
...
subplot(3,1,3);
F_methane = fftshift(fft(methane_P1));
...
ylabel('Amplitude');
%% Number of peaks
peak_h10 = sum(F_mag_h10(pos_val)>0.2);
peak_h50 = sum(F_mag_h50(pos_val)>0.2);
peak_methane = sum(F_mag_methane(pos_val)>0.2);
%% Spectrogram
figure(3)
spectrogram(h50_P1,1500,[],[],10000,'yaxis');
colormap jet;
set(gca,'FontSize',18,'Linewidth',1.5);
title("Spectrogram of P1, Hydrogen10");
%% Wavelet transform
[h10_P1_wt,h10_P1_freq] = cwt(h10_P1,10000);
[h50_P1_wt,h50_P1_freq] = cwt(h50_P1,10000);
[methane_P1_wt,methane_P1_freq] = cwt(methane_P1,10000);


figure(4)
surface(t,h10_P1_freq,abs(h10_P1_wt));
shading flat
axis tight
set(gca,"yscale","log")
colormap jet
title('scalogram of P1 data for Hydrogen 10');
colorbar

figure(5)
surface(t,h50_P1_freq,abs(h50_P1_wt));
shading flat
axis tight
set(gca,"yscale","log")
colormap jet
title('scalogram of P1 data for Hydrogen 50');
colorbar

figure(6)
surface(t,methane_P1_freq,abs(methane_P1_wt));
shading flat
axis tight
set(gca,"yscale","log")
colormap jet
title('scalogram of P1 data for Methane');
colorbar